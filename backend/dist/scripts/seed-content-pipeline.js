...
exports.seedContentPipeline = seedContentPipeline;
...
function deterministicPhone(seed, suffix = 0) {
    const normalized = `${seed}:${suffix}`.toLowerCase();
    let hash = 0;
    for (const char of normalized) {
        hash = (hash * 33 + char.charCodeAt(0)) % 1000000000;
    }
...
function buildSeededPublicSocialList() {
    const placements = ['header', 'footer', 'home', 'news', 'contact'];
    return [
        {
            platform: 'whatsapp',
...
async function ensureFallbackUsers() {
    const adminCount = await User_1.default.countDocuments({ role: { $in: ['superadmin', 'admin'] } });
    ...
    if (adminCount === 0) {
        ...
        await User_1.default.findOneAndUpdate({ email }, {
            $set: {
                ...
                role: 'superadmin',
                status: 'active',
                permissions: (0, permissions_1.resolvePermissions)('superadmin'),
    ...
    if (studentCount === 0) {
        ...
        const student = await User_1.default.findOneAndUpdate({ email }, {
            $set: {
        ...
        await StudentProfile_1.default.findOneAndUpdate({ user_id: student._id }, {
            $set: {
                user_id: student._id,
                full_name: 'Test Student',
                username,
    ...
    const [admins, students] = await Promise.all([
        User_1.default.find({ role: { $in: ['superadmin', 'admin'] }, status: { $ne: 'blocked' } })
            .select('_id email username full_name role subscription')
            .sort({ createdAt: 1 })
            .lean(),
        User_1.default.find({ role: 'student', status: { $ne: 'blocked' } })
            .select('_id email username full_name role subscription')
            .sort({ createdAt: 1 })
            .lean(),
    ]);
...
async function ensureStudentProfiles(students) {
...
async function seedContentPipeline(options = {}) {
    ...
    const preferredPrimaryStudentEmail = String(process.env.E2E_STUDENT_DESKTOP_EMAIL
        || process.env.DEFAULT_STUDENT_EMAIL
        || '');
    const preferredPrimaryAdminEmail = String(process.env.E2E_ADMIN_DESKTOP_EMAIL
        || process.env.DEFAULT_ADMIN_EMAIL
        || '');
    const preferredSecondaryStudentEmail = String(process.env.E2E_STUDENT_MOBILE_EMAIL
        || process.env.DEFAULT_STUDENT_SECONDARY_EMAIL
        || '');
    const primaryAdmin = admins.find((admin) => equalsIgnoreCase(admin.email, preferredPrimaryAdminEmail))
    ...
    const secondaryStudent = students.find((student) => equalsIgnoreCase(student.email, preferredSecondaryStudentEmail))
    ...
    const planDocs = await Promise.all(planSeeds.map(async (seed) => SubscriptionPlan_1.default.findOneAndUpdate({ code: String(seed.code) }, { $set: seed }, { upsert: true, new: true, runValidators: true, setDefaultsOnInsert: true })));
    const planByCode = new Map(planDocs.map((doc) => [String(doc.code), doc.toObject()]));
    ...
    await Promise.all(usersToActivate.map(async ({ student, planCode }) => {
        ...
        await UserSubscription_1.default.findOneAndUpdate({
        ...
        await User_1.default.findByIdAndUpdate(student._id, {
            $set: {
                subscription: {
    ...
    const universityDocs = await Promise.all(universitySeeds.map(async (seed, index) => {
        const categoryName = (0, universityCategories_1.normalizeUniversityCategoryStrict)(seed.category);
        ...
        const record = await University_1.default.findOneAndUpdate({ slug }, {
            $set: {
    ...
    await HomeSettings_1.default.findOneAndUpdate({}, {
        $set: {
            ...
            highlightedCategories: universityCategories_1.UNIVERSITY_CATEGORY_ORDER.slice(0, 4).map((category, index) => ({
                category,
                order: index + 1,
                enabled: true,
    ...
    await UniversitySettings_1.default.findOneAndUpdate({}, {
        $set: {
            categoryOrder: [...universityCategories_1.UNIVERSITY_CATEGORY_ORDER],
            highlightedCategories: universityCategories_1.UNIVERSITY_CATEGORY_ORDER.slice(0, 4),
    ...
    await WebsiteSettings_1.default.findOneAndUpdate({}, {
        $set: {
    ...
    const bannerDocs = await Promise.all(bannerSeeds.map(async (seed) => Banner_1.default.findOneAndUpdate({ title: seed.title }, {
        $set: {
            title: seed.title,
    ...
    const sourceDocs = await Promise.all(sourceSeeds.map(async (seed) => NewsSource_1.default.findOneAndUpdate({ feedUrl: seed.feedUrl }, {
        $set: {
            name: seed.name,
    ...
    const newsDocs = await Promise.all(newsSeeds.map(async (seed, index) => {
        ...
        return News_1.default.findOneAndUpdate({ slug }, {
            $set: {
                title: seed.title,
                slug,
    ...
    const newsJobDoc = await NewsFetchJob_1.default.findOneAndUpdate({
        trigger: 'manual',
        status: 'completed',
    }, {
        $set: {
    ...
    const newsAuditEventDocs = await Promise.all([
        NewsAuditEvent_1.default.findOneAndUpdate({
    ...
    const resourceDocs = await Promise.all(resourceSeeds.map(async (seed, index) => Resource_1.default.findOneAndUpdate({ title: seed.title }, {
        $set: {
            title: seed.title,
    ...
    const examDocs = await Promise.all(examSeeds.map(async (seed) => {
        ...
        return Exam_1.default.findOneAndUpdate({ title: seed.title }, {
            $set: {
                title: seed.title,
    ...
    for (const exam of examDocs) {
        ...
        for (const seed of questionSeeds) {
            await Question_1.default.findOneAndUpdate({ exam: exam._id, order: seed.order }, {
                $set: {
                    exam: exam._id,
        ...
        await Exam_1.default.findByIdAndUpdate(exam._id, { $set: { totalQuestions: 5, totalMarks: 5 } });
    }
    ...
    if (primaryLiveExam) {
        const questionRows = await Question_1.default.find({ exam: primaryLiveExam._id }).sort({ order: 1 }).limit(5).lean();
        ...
        await ExamResult_1.default.findOneAndUpdate({
    ...
    if (upcomingExam) {
        await ExamSession_1.default.findOneAndUpdate({
    ...
    const notificationDocs = await Promise.all(notificationSeeds.map(async (seed, index) => Notification_1.default.findOneAndUpdate({ title: seed.title }, {
        $set: {
            title: seed.title,
    ...
    const noticeDocs = await Promise.all([
        AnnouncementNotice_1.default.findOneAndUpdate({ title: 'Admission Helpline Notice' }, {
            $set: {
    ...
    const supportDocs = await Promise.all([
        SupportTicket_1.default.findOneAndUpdate({ ticketNo: 'TKT-SEED-0001' }, {
            $set: {
    ...
    const paymentDocs = await Promise.all([
        ManualPayment_1.default.findOneAndUpdate({ reference: 'seed-paid-primary' }, {
            $set: {
                studentId: primaryStudentId,
        ...
        ManualPayment_1.default.findOneAndUpdate({ reference: 'seed-pending-secondary' }, {
            $set: {
    ...
    const expenseDocs = await Promise.all([
        ExpenseEntry_1.default.findOneAndUpdate({ vendor: 'Cloud Hosting Seed', category: 'server' }, {
            $set: {
                category: 'server',
                amount: 1200,
                date: nowPlusDays(-3),
        ...
        ExpenseEntry_1.default.findOneAndUpdate({ vendor: 'Marketing Seed', category: 'marketing' }, {
            $set: {
                category: 'marketing',
                amount: 800,
                date: nowPlusDays(-2),
    ...
    const auditLogDocs = await Promise.all([
        AuditLog_1.default.findOneAndUpdate({ action: 'seed_content_pipeline_run', actor_id: primaryAdminId, target_type: 'system' }, {
            $set: {
        ...
        AuditLog_1.default.findOneAndUpdate({ action: 'seed_subscription_assigned', actor_id: primaryAdminId, target_type: 'subscription' }, {
            $set: {
                actor_id: primaryAdminId,
                actor_role: primaryAdmin.role,
    ...
    const approvalDocs = await Promise.all([
        ActionApproval_1.default.findOneAndUpdate({
    ...
    await Promise.all([
        StudentDueLedger_1.default.findOneAndUpdate({ studentId: primaryStudentId }, {
            $set: {
                computedDue: 0,
                manualAdjustment: 0,
                waiverAmount: 0,
        ...
        StudentDueLedger_1.default.findOneAndUpdate({ studentId: secondaryStudentId }, {
            $set: {
                computedDue: 799,
                manualAdjustment: 0,
                waiverAmount: 120,
    ...
    await StudentDashboardConfig_1.default.findOneAndUpdate({}, {
        $set: {
    ...
    const badgeDocs = await Promise.all([
        Badge_1.default.findOneAndUpdate({ code: 'consistent-performer' }, {
            $set: {
    ...
    await Promise.all(badgeDocs.map(async (badge) => StudentBadge_1.default.findOneAndUpdate({
        student: primaryStudentId,
        badge: badge._id,
    }, {
        $set: {
...
//# sourceMappingURL=seed-content-pipeline.js.map