...
function buildDataLogo(label, bg) {
    const svg = `
...
async function upsertUniversity(seed) {
    ...
    const doc = await University_1.default.findOneAndUpdate({ slug }, {
        $set: {
            name: seed.name,
    ...
    if (seed.rawShortForm !== undefined) {
        await University_1.default.collection.updateOne({ _id: doc._id }, { $set: { shortForm: seed.rawShortForm } });
    }
    return doc._id;
}
async function run() {
    try {
        ...
        const admin = await User_1.default.findOne({ role: { $in: ['superadmin', 'admin'] } }).sort({ createdAt: 1 }).lean();
        const adminId = admin?._id ? new mongoose_1.default.Types.ObjectId(String(admin._id)) : null;
        ...
        for (let index = 0; index < highlightedCategories.length; index += 1) {
            ...
            await UniversityCategory_1.default.findOneAndUpdate({ name }, {
                $set: {
                    name,
                    slug: toSlug(name),
                    isActive: true,
        ...
        const engineeringCenters = (0, universitySyncService_1.normalizeExamCenters)('Dhaka - BUET Campus | Rajshahi - RUET Campus | Khulna - KUET Campus');
        const gstCenters = (0, universitySyncService_1.normalizeExamCenters)('Sylhet - SUST Campus | Dinajpur - HSTU Campus');
        const medicalCenters = (0, universitySyncService_1.normalizeExamCenters)('Dhaka - DMC Campus | Chattogram - CMC Campus');
        const fixtureIds = [];
        const universities = [
            {
            ...
            },
            {
                ...
                examCenters: (0, universitySyncService_1.normalizeExamCenters)('Dhaka - Curzon Hall'),
            },
            {
                ...
                examCenters: (0, universitySyncService_1.normalizeExamCenters)('Savar - JU Main Campus'),
            },
            {
            ...
            },
            {
                ...
                examCenters: (0, universitySyncService_1.normalizeExamCenters)('Dhaka - QA North Campus'),
            },
            {
                ...
                examCenters: (0, universitySyncService_1.normalizeExamCenters)('Khulna - QA South Campus'),
            },
            {
            ...
            },
            {
                ...
                examCenters: (0, universitySyncService_1.normalizeExamCenters)('Sylhet - QA East Campus'),
            },
            {
        ...
        const clusterSeeds = [
            ...
            },
            {
                ...
                dates: {
                    ...
                    examCenters: (0, universitySyncService_1.normalizeExamCenters)('Dhaka - Curzon Hall | Savar - JU Campus'),
                },
            },
            {
        ...
        const clusterIdsByName = new Map();
        for (const seed of clusterSeeds) {
            const ensured = await (0, universitySyncService_1.ensureUniversityClusterByName)(seed.name);
            clusterIdsByName.set(seed.name, ensured._id);
            await UniversityCluster_1.default.findByIdAndUpdate(ensured._id, {
                $set: {
            ...
            const memberIds = await University_1.default.find({ name: { $in: seed.members } }).distinct('_id');
            await (0, universitySyncService_1.syncManualClusterMembership)(memberIds, String(ensured._id));
            if (memberIds.length > 0) {
                await University_1.default.updateMany({ _id: { $in: memberIds } }, {
                    $set: {
            ...
            await (0, universitySyncService_1.syncUniversityClusterSharedConfig)(String(ensured._id), adminId ? String(adminId) : null);
        }
        await (0, universitySyncService_1.reconcileUniversityClusterAssignments)(adminId ? String(adminId) : null);
        const featuredUniversities = await University_1.default.find({ slug: { $in: [
                    'bangladesh-university-of-engineering-and-technology',
                    'rajshahi-university-of-engineering-technology',
                    'khulna-university-of-engineering-technology',
                    'shahjalal-university-of-science-and-technology',
            ...
            .select('_id slug')
            .lean();
        const homeDefaults = (0, HomeSettings_1.createHomeSettingsDefaults)();
...
void run();
//# sourceMappingURL=e2e_prepare_open_universities.js.map