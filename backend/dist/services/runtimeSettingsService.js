...
exports.getDefaultRuntimeFeatureFlags = getDefaultRuntimeFeatureFlags;
exports.getRuntimeSettingsSnapshot = getRuntimeSettingsSnapshot;
...
function normalizeFeatureFlags(source, strictExamTabLock) {
    const raw = source || {};
    return {
        ...
        requireDeleteKeywordConfirm: asBoolean(raw.requireDeleteKeywordConfirm, DEFAULT_FEATURE_FLAGS.requireDeleteKeywordConfirm),
    };
}
function getDefaultRuntimeFeatureFlags() {
    return { ...DEFAULT_FEATURE_FLAGS };
}
async function getRuntimeSettingsSnapshot(forceRefreshSecurity = true) {
    let settings = await Settings_1.default.findOne();
    if (!settings) {
        settings = await Settings_1.default.create({});
    }
    const security = await (0, securityConfigService_1.getSecurityConfig)(forceRefreshSecurity);
    const featureFlags = normalizeFeatureFlags(settings.featureFlags, security.strictExamTabLock);
    const runtimeVersion = asPositiveInt(settings.runtimeVersion, 1);
    const needsPatch = settings.runtimeVersion !== runtimeVersion ||
    ...
    if (needsPatch) {
        await Settings_1.default.updateOne({ _id: settings._id }, {
            $set: {
                featureFlags,
                runtimeVersion,
            },
        });
        settings = await Settings_1.default.findById(settings._id);
    }
    return {
        security,
        featureFlags,
...
async function updateRuntimeSettings(input) {
    ...
    await Settings_1.default.findOneAndUpdate({}, { $set: updateDoc }, { upsert: true, new: true, setDefaultsOnInsert: true });
    if (input.security) {
        const admin2fa = input.security.enable2faAdmin !== undefined
        ...
        await (0, securityCenterService_1.updateSecuritySettingsSnapshot)({
...
//# sourceMappingURL=runtimeSettingsService.js.map