...
async function adminGetNotices(req, res) {
    try {
        ...
        if (sourceNewsId && mongoose_1.default.Types.ObjectId.isValid(sourceNewsId)) {
            filter.sourceNewsId = new mongoose_1.default.Types.ObjectId(sourceNewsId);
        }
        ...
        const notice = await AnnouncementNotice_1.default.create({
            ...
            classification: {
                ...
                tags: Array.isArray(classification.tags)
                    ? classification.tags.map((item) => String(item).trim()).filter(Boolean)
                    : [],
