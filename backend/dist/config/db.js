...
async function ensureCriticalIndexes() {
    ...
    try {
        ...
        const legacyUnique = resultIndexes.find((idx) => idx.name === 'exam_1_student_1'
            && idx.unique === true
            && idx.key?.exam === 1
            && idx.key?.student === 1);
        if (legacyUnique) {
        ...
        const hasCurrentUnique = resultIndexes.some((idx) => idx.unique === true
            && idx.key?.exam === 1
            && idx.key?.student === 1
            && idx.key?.attemptNo === 1);
        if (!hasCurrentUnique) {
            await results.createIndex({ exam: 1, student: 1, attemptNo: 1 }, { unique: true, name: 'exam_1_student_1_attemptNo_1' });
