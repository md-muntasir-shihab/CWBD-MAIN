...
import { markAdminAlertsRead, queryAdminAlerts } from '../services/adminAlertService';

function ensureAlertAdmin(req: AuthRequest, res: Response): 'superadmin' | 'admin' | 'moderator' | null {
    if (!req.user) {
    ...
    return req.user.role as 'superadmin' | 'admin' | 'moderator';
}
